...
import top.jrhong.library.entitys.Book;
import top.jrhong.library.entitys.Borrow;
import top.jrhong.library.entitys.Log;
import top.jrhong.library.entitys.User;
import top.jrhong.library.repository.BookRepository;
import top.jrhong.library.repository.BorrowRepository;
import top.jrhong.library.repository.LogRepository;
...
public class BorrowHandler {
    ...
    @GetMapping("/{page}/{size}/user/{idCard}")
    public Page<Borrow> getPageBookUser(@PathVariable("page") Integer page, @PathVariable("size") Integer size, @PathVariable("idCard") String idCard) {
        PageRequest pageable = PageRequest.of(page - 1, size);
        // 只返回当前登录用户的借阅信息
